...
export default function InactiveComponentsPage() {
    ...
    useEffect(() => {
        if (status === "unauthenticated") {
            ...
            fetchInactiveComponents();
        }
    }, [status, searchQuery, filterType, filterStatus, currentPage]);

    const fetchInactiveComponents = async () => {
        try {
            setLoading(true);
            // Bắt đầu với status là dưới một trong hai trạng thái không hoạt động
            let url = `/api/components?page=${currentPage}&status=${filterStatus || 'under_repair,disposed'}`;
    ...
    const getStatusName = (status: string) => {
        const foundStatus = inactiveStatuses.find(s => s.value === status);
        ...
        <div className="min-h-screen flex flex-col">
            <header className="bg-primary text-primary-foreground shadow-md">
                <div className="container mx-auto py-4 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        ...
                        <span>/</span>
                        <h1 className="text-xl font-bold">Linh kiện không hoạt động</h1>
                    </div>
            ...
            <main className="flex-1 container mx-auto py-6 px-4">
                <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h2 className="text-2xl font-bold">Linh kiện không hoạt động</h2>
                        <p className="text-muted-foreground">Đang sửa chữa hoặc đã thanh lý</p>
                    </div>
                ...
                <div className="bg-card border rounded-lg overflow-hidden">
                        ...
                        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-3 items-end">
                            ...
                            <div className="w-full md:w-48">
                                ...
                                <select
                                    ...
                                    className="w-full px-3 py-2 border rounded-md bg-background"
                                >
                                    <option value="">Tất cả không hoạt động</option>
                    ...
                    {loading ? (
                        ...
                        <div className="p-8 text-center">
                            <p className="text-muted-foreground">Không tìm thấy linh kiện không hoạt động nào.</p>
                        </div>
                    ) : (
